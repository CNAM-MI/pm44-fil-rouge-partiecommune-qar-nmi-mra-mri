    ...
    public class OsmApiService
    {
        ...
        private static string GetUrlParamsForList(GeoPoint? center, uint? kmRadius, uint page)
        {
            if (kmRadius.HasValue && center != null)
                return BASE_SELECTION + $"&where=within_distance(meta_geo_point%2C%20geom%27POINT({center.lon}%20{center.lat})%27%2C%20{kmRadius}km)&limit={PAGE_SIZE}&offset={PAGE_SIZE * page}";
